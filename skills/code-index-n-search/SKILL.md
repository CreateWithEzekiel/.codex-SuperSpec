...
- keep all generated index files inside `.repo_executive_context/code_index_n_search`
...
`.repo_executive_context/code_index_n_search/`
...
- `auto`: inspect `.repo_executive_context/code_index_n_search/index_state.json`; use `direct` only when it records `write_mode: direct`, otherwise use `atomic`
...
script = codex_home / "skills" / "code-index-n-search" / "scripts" / "query_index.py"
...
```powershell
python <codex-home>/skills/code-index-n-search/scripts/build_index.py
```
